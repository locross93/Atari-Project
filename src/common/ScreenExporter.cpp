...
void ScreenExporter::saveReward(int reward) {
    ...
    // actionString << m_frame_number << ":" << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count() << ":" << action << std::endl;
...
void ScreenExporter::saveRAM(const ALERAM &RAM ) {

    // Must have specified a directory. 
    assert(m_path.size() > 0);

    std::ostringstream outFileName;
    std::ostringstream RAMString;
    ...
    outFileName << m_path << "/" <<  "RAM.txt";
    RAMString << m_frame_number << ":";
    ...
    std::ofstream RAMFile(outFileName.str(), std::ios_base::app);
    RAMFile << RAMString.str();
    RAMFile.close();
}
